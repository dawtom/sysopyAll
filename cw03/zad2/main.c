...
int main(int argc, char ** argv){
    ...
    unsigned long int time_limit = (unsigned)atoi(argv[2]);
    unsigned long int memory_limit =(unsigned)atoi(argv[3]);



    (*processor_time_limiter).rlim_max = time_limit;
    (*virtual_memory_limiter).rlim_max = memory_limit;
    (*processor_time_limiter).rlim_cur = time_limit;
    (*virtual_memory_limiter).rlim_cur = memory_limit;

    printf("\n%li %li\n", processor_time_limiter->rlim_max,processor_time_limiter->rlim_cur);
        ...
        if (mode == EXECUTE_PROGRAM)
        {
            if (parent_PID == getpid()){
                ...
                if (child_PID == 0) {
                    ...
                    printf("\nSystime: %lld Utime:%lld\n",
                           (long long) usage_children.ru_utime.tv_sec,(long long) usage_children.ru_stime.tv_sec);
                }
            }
        } else{
