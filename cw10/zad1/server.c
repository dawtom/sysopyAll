            ...
            else if (sfd == events[i].data.fd)
            {
                ...
                while (1)
                {
                    ...
                    if (s == 0)
                    {
                        ...
                        client_descriptors[client_count] = infd;
                        client_count++;
...
void * computing_thread_function(void *arg){
    while (1) {
        ...
        printf("Client descriptors:\n");
        for (int i = 0; i < MAX_CLIENTS_NUMBER; ++i) {
            printf("%d ", client_descriptors[i]);
        }
        printf("\n");
...
// ./server 127.0.0.1 un_server
// ./inet_client cl inet 127.0.0.1 7891